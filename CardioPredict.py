...
def load_assets():
    ...
    scaler_path = "scaler.pkl" # Add your scaler file here if you have one
...
if model:
    ...
    with tabs[0]:
        ...
        if eval_btn:
            ...
            # Application of Scaler if available
            if scaler:
            ...
            # 4. Determine Risk Category (WITH OVERRIDES)
            risk_df = explain_cvd_risks(user_vals)
            num_clinical_risks = len(risk_df) if risk_df is not None else 0

            # Logic: Even if AI is unsure, high WHO risk factors force a higher category
            ...
            st.markdown(f"""
                <div style="background-color: {bg}; color: {color}; padding: 20px; border-radius: 8px; border: 1px solid {color}; margin-bottom: 10px;">
