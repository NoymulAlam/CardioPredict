...
if model:
    ...
    with tabs[0]:
        ...
        if eval_btn:
            ...
            st.write("Raw data shape:", input_df.shape)
            st.write("Raw data frame:", input_df)
            
            if scaler:
                input_processed = scaler.transform(input_df)
                st.write("Scaled data (sent to model):", input_processed)
            else:
                input_processed = input_df.values
                st.warning("⚠️ No scaler found. Sending raw values to the model.")
            ...
            st.markdown(f"""
                <div style="background-color: {bg}; color: {color}; padding: 20px; border-radius: 8px; border: 1px solid {color}; margin-bottom: 10px; margin-top: 20px;">
